...
class TestEnumerables < MiniTest::Unit::TestCase
  ...
  def test_find_or_detect_method
  ...
  def test_inject_method_as_accumulator
    assert_equal 25, @ary1.inject { |sum, e| sum + e }
  end

  def test_inject_method_as_accumulator_with_initial_value
    assert_equal 30, @ary1.inject(5) { |sum, e| sum + e }
  end

  def test_inject_method_as_comparator
